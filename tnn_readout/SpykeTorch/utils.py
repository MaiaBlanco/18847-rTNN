...
class OnKernel(FilterKernel):
    def __init__(self, window_size=3):
        super(OnKernel, self).__init__(window_size)
    ...
    def __call__(self):
        ...
        onfilter = np.ones((self.window_size,self.window_size))*-1.0
        onfilter[mid][mid] = (self.window_size**2)-1
        ontensor = torch.from_numpy(onfilter)
        return ontensor.float()
    
class OffKernel(FilterKernel):
    def __init__(self, window_size=3):
        super(OffKernel, self).__init__(window_size)

    # returns a 2d tensor corresponding to the requested Off filter
    def __call__(self):
        #offfilter = np.array([[1.0,1.0,1.0],[1.0,-8.0,1.0],[1.0,1.0,1.0]])
        mid = int((self.window_size-1)/2)
        offfilter = np.ones((self.window_size,self.window_size))
        offfilter[mid][mid] = 1-(self.window_size**2)
...
class Crop:
    ...
    def __call__(self, image):
        return image[:,:,self.start:self.start+self.size,self.start:self.start+self.size]
